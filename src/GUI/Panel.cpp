...
Panel::Panel(){
    ...
    this->panelHolder = {0,650-(panelButtonSize.y/2.0f-panelButtonFontSize/2.0f),panelButtonSize.x+(panelButtonSize.y-panelButtonFontSize),350};
    this->subPanelHolder = {panelHolder.width,650-(panelButtonSize.y/2.0f-panelButtonFontSize/2.0f),400-panelHolder.width,350};
    Back.deActive();
}
...
void Panel::drawPanel(){
    ...
    if(isUsing){
        ...
        DrawRectangleRoundedLinesEx(subPanelHolder, 0, 0, 3, outlineButtonColor);
    }
    ...
    DrawRectangleRoundedLinesEx(panelHolder,0,0,3,outlineButtonColor);
}
